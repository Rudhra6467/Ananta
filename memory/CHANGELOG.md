...
### 2026-06-XX — Phase C (My Reports History) + Cockpit declutter
- **My Reports History (Phase C, tested iter64):** Rewrote shared `AnantaPdfs.jsx` from a browser-local `pdfRegistry` list to a SERVER-BACKED history reading `GET /api/lab/runs`. Columns: strategy names + kind, DONE/RUNNING/QUEUED/FAILED status badge, TF (+MTF)/exit-method/period/asset chips, date, and View-Download PDF (`/api/lab/runs/{id}/pdf`) + Ask-Ananta-analyse + Delete (`DELETE /api/lab/runs/{id}`). Loading/empty/error/owner-gate states; polls while runs in-flight; auto-refreshes on run completion. Automated daily `health_sweep` runs are filtered out (they have their own HEALTH dashboard). Persists across devices/sessions. Renders in Research › AI ANALYSIS, Exit Engine › AI ANALYSIS (Workspace › AI ANALYTICS too, though Workspace is not currently routed in AppShell). 13/13 backend tests (`tests/test_lab_runs_history.py`).
- **Cockpit declutter (per owner):** Removed the SETUPS/SCANNED/REJECTED/QUALIFIED 4-box `BotBrainStrip` from the Cockpit (Dashboard.jsx) — reduced alarm/visual noise on the landing tab. Header, START TRADING/WEEKLY AI REVIEW, Trade Life Cycle, Analytics, Position Tracker all unchanged. (Future: surface qualified-setups data in PDF reports instead.)

### 2026-06-XX — Cockpit "Strategy Health Today" card
- Added a compact **Strategy Health Today** card to the Cockpit (Dashboard.jsx), in the space vacated by the removed 4-box block. Reads the daily health sweep (`GET /api/lab/health`), shows the **top 2 recommended** strategies (recommendation.tone==positive, sorted by backtest net P&L desc) as cards: strategy icon + name + recommendation badge (same language as the Health Dashboard, e.g. "Recommended for Paper Trading") + backtest window net P&L. Header has a **View Health Dashboard** button that navigates to Research › HEALTH. Empty state ("No strategies currently recommended for paper trading.") when none qualify. No letter grades (consistent with the earlier Strategy Center decision). Verified via screenshots (empty + populated states) and navigation click.

### 2026-06-XX — "Copy Existing" (Clone) in Add Strategy modal (iter65)
- **Backend:** `POST /api/library/{id}/clone` (owner-only) duplicates a RULE-BASED (declarative) strategy into a new independently-editable library entry — fresh `engine_key`, wired via the declarative engine like an import (rehydrated on startup), `origin=clone`, `cloned_from` set, name "<Name> (Copy)", metrics/history reset. Core engine strategies (hunter/squeeze/continuation, internal) return **400** (built-in logic, cannot be copied). Missing → 404, unauth → 401.
- **Frontend:** Strategy Center › Add now has 3 options — Create New / **Copy Existing** / Import JSON. "Copy Existing" opens `CloneStrategyPicker` listing cloneable rule-based strategies (core excluded); tapping one clones via `api.libraryClone`, shows a success toast, and refreshes the grid so the new "(Copy)" card appears.
- **Tested:** iter65 — 5/5 backend pytest (`tests/test_iter65_library_clone.py`) + web UI flow (grid 16→17, toast). Test clones cleaned from DB.
- **Known gap (not requested):** no UI/endpoint yet to delete a cloned library strategy.

### 2026-06-XX — Manage user-added strategies (rename + delete)
- **Backend:** `PATCH /api/library/{id}` (rename) and `DELETE /api/library/{id}` — restricted to user-added strategies (`imported` or `origin=clone`); built-in Core + seeded Catalog are locked (400). Rename keeps the runtime registry name in sync for wired declarative strategies. Delete removes the library entry + its `strategy_configs` + `strategy_meta`, and unregisters the imported engine. Delete is blocked with **409** while the strategy is effectively deployed (mirrors the live-engine gate: enabled AND status not DISABLED/ERROR).
- **Clone fix:** fresh clones now start **DISABLED** (a copy must not auto-trade; the live engine treats a missing meta row as enabled-by-default, so clones now pin `strategy_meta{enabled:false,status:DISABLED}`). This makes display, live-engine behaviour, and the delete guard consistent.
...
### 2026-06-XX — MOBILE PARITY Phase 1 (Cockpit health + Research Health + Reports history)
- **Mobile api.ts:** added labHealth/labHealthStatus/labHealthSweep, labRuns, labRunPdfUrl, deleteLabRun, libraryClone/libraryRename/libraryDelete (shared backend, EXPO_PUBLIC_BACKEND_URL + /api).
- **Cockpit ((tabs)/index.tsx):** removed the SETUPS/SCANNED/REJECTED/QUALIFIED 4-box grid (mirrors web declutter) and added a **Strategy Health** card (`StrategyHealthCard.tsx`) — top-2 recommended strategies side-by-side (icon + name + recommendation badge + net P&L), a count badge, empty state, and a **View Health** button → `/research?sub=health`.
- **Research ((tabs)/research.tsx):** new **HEALTH** sub-tab (`Health` panel) reading GET /api/lab/health with a RUN SWEEP button (POST /api/lab/health/sweep) and per-strategy cards (recommendation badge, Net P&L/Win/PF/Trades, Best Timeframe/Exit, MFE Capture, Best Regime, "based on backtest" note). **Reports** sub-tab now leads with server-backed **My Reports History** (GET /api/lab/runs, excludes health_sweep, open PDF via Linking, delete via DELETE /api/lab/runs/{id}) above the existing Trade PDF reports.
- **Verified** on Expo web preview: cockpit card (empty state matches live data), Health tab (6 strategies analysed with badges/metrics), Reports (22 validation rows). Lint clean (only pre-existing api default-import warnings).
- **Remaining mobile parity (Phase 2+):** redesigned Strategy Center grid + detail; Add-strategy Copy Existing (clone) picker; Rename/Delete controls; Test in Research Lab quick-action; Research deep analytics (MFE/MAE, regime tables, exit compare) + multi-timeframe results.

### 2026-06-XX — MOBILE PARITY Phase 2 (Copy Existing + Rename/Delete + Test in Lab)
- **AddStrategySheet.tsx:** added a **Copy Existing** option that opens an in-sheet clone picker listing rule-based strategies (wireable && !internal && !reference_only); tap → POST /api/library/{id}/clone, then refreshes the library. Core strategies excluded.
- **library/[id].tsx:** for user-added strategies (imported or origin=clone) added inline **Rename** (pencil → TextInput + save/cancel, PATCH /api/library/{id}) and **Delete** (trash → confirm Alert, DELETE /api/library/{id}, blocked while deployed). Added a **COPY** badge for clones (Imported badge hidden for clones), and a **Test in Lab** button on wired strategies → /research?sub=validate&strat={engine_key} (preselects the Validate wizard). Guarded the Backtest Performance card so fresh clones (null historical_results) show "Not yet validated" instead of "undefined
### 2026-06-XX — MOBILE PARITY Phase 2 (Copy Existing + Rename/Delete + Test in Lab)
- AddStrategySheet.tsx: added a "Copy Existing" option opening an in-sheet clone picker listing rule-based strategies (wireable && !internal && !reference_only); tap -> POST /api/library/{id}/clone, then refreshes the library. Core strategies excluded.
- library/[id].tsx: for user-added strategies (imported or origin=clone) added inline Rename (pencil -> TextInput + save/cancel, PATCH /api/library/{id}) and Delete (trash -> confirm Alert, DELETE /api/library/{id}, blocked while deployed). Added a COPY badge for clones (Imported badge hidden for clones), and a Test in Lab button on wired strategies -> /research?sub=validate&strat={engine_key}. Guarded the Backtest Performance card so fresh clones (null historical_results) show "Not yet validated" instead of undefined percentages.
...
### 2026-06-19 — MOBILE Strategy Center grid redesign (web parity)
- Rewrote /app/mobile/app/(tabs)/strategy.tsx to match the redesigned web StrategyCenter: removed the DEPLOYED/EDIT Segmented control, removed the StrategyLeaderboard (podium/sort/grades), removed the promotional quick-filter chips row. Replaced the single-column list with a clean flat 2-column grid (testID='strategy-grid') of cards: icon + status Pill + name + 2-line description + activity line ("N trades · last Xh ago" / style) + footer action (DEPLOY / MANAGE / VIEW / dashed reference note). No AI-grade circle on cards. Kept inline search + Filter drawer (favorites + facets).
...
## UI cleanup: duplicate Stop button, manual-save Risk Monitor, wizard live toggle (2026-07)
- ISSUE 1 — Removed the duplicate "STOP ANANTA" button from the Risk Monitor · Safeguards card on BOTH web (RiskMonitorPanel.jsx) and mobile (workspace.tsx). The only Stop control is now the page-header one (web workspace-stop-ananta / mobile ee-stop-ananta).
...
## P2 — Custom (imported/cloned) strategies executable in the Research Lab (2026-06 / launch+1)
ROOT CAUSE: custom strategies (AI-import declarative + clones) already ran LIVE (trading_engine uses all_declarative_keys() in the main process, where the imported registry is rehydrated on startup) and had working standalone backtests (import backtest-preview / library backtest via declarative_backtest). BUT the full Research Lab validation flow silently dropped them: lab/backtest.run_backtest only looked up the hardcoded DECLARATIVE catalog, and the Lab executes in an isolated worker PROCESS (ProcessPoolExecutor) that never rehydrates the in-memory _IMPORTED registry. Selecting a custom strategy for a Lab run produced 0 entries → blank result.
FIX (no engine rewrite needed — the declarative executor already handles arbitrary specs):
- lab/backtest.run_backtest + run_multi_exit accept a picklable `decl_overrides` = {lowercased_engine_key: {spec, params}}; used for any selected key not in the hardcoded catalog (catalog keys still use their default params).
- lab/runner.LabWorker._resolve_custom_specs(strategies): resolves each selected custom key's declarative_spec (from strategy_library) + params (strategy_runtime.resolve_full_params, honoring the active config) in the PARENT process and passes them into every worker cell — backtest, exit-comparison, and health-sweep.
Verified: pytest tests/test_lab_custom_strategy_wiring.py (3/3: custom executes, dropped without overrides, multi-exit forwards); existing test_lab_declarative_wiring.py 9/9 (no regression); END-TO-END via API — cloned ema-cross ("clone-*") ran a full Lab backtest = 29 entries/29 trades, all attributed to the custom key (was 0 before). Live/watcher/preview paths unchanged (already worked).
NOTE: imports that DON'T compile to the declarative engine (declarable=false) remain reference-only by design — they cannot run natively and are flagged in import validation.

## Fix: deleting an imported/cloned strategy now leaves /strategy/registry (2026-06, iter82)
BUG (found by testing agent during P2 verification): DELETE /api/library/{key} returned {deleted:true} but the strategy still appeared in /api/strategy/registry — unregister_imported() only popped _IMPORTED and never removed the schema from the REGISTRY that register_imported() populated. Impact: orphan clones cluttered the web+mobile Research picker forever; picking an orphan → silent 0-entry Lab run.
FIX: added strategy.core.unregister(key) (removes all versions from REGISTRY + _LATEST); unregister_imported() now calls it. Verified: unit test (test_lab_custom_strategy_wiring.py::test_unregister_imported_removes_from_registry), pre-existing orphans cleared on restart (rehydration only re-registers existing library imports), and runtime clone→delete→registry confirms the key is gone (present after clone: True, after delete: False).

## Web parity fixes: Strategy Center sections + Import "Save without AI" (2026-06, iter84)
Testing (iter83) found two web surfaces lagged mobile P1 parity. Fixed WEB only:
1. StrategyCenter.jsx: library grid now splits into "LIVE / PAPER" (actively running) and "TEST & EDIT" sections via the same isLiveMetric(m) predicate mobile uses (m.enabled && status not DISABLED/ERROR). Empty sections auto-hide. testIDs strategy-section-live / strategy-section-test (grid keeps data-testid strategy-grid).
2. ImportStrategyModal.jsx + api.js: added "Save without AI" button (data-testid import-direct-btn) beside "Analyze with AI"; calls new web api.importDirect -> POST /api/library/import/direct (no LLM) and lands on the Review step. Mutual-disable while busy.
Verified: lint clean; sections render (screenshot: both present); testing agent code-review PASS on both; direct-import path e2e via curl (draft ai_skipped=True, then deleted). Live click-through of the modal was blocked only by the shared-preview onboarding overlay chain (harness friction, not a product bug).

## Fix: "Save without AI" 500 + custom strategies stuck view-only (2026-07-23, prod-reported)
User (on PRODUCTION) hit "Internal server error" saving a free-form JSON via Add Strategy > Import > "Save without AI", and complained added strategies were view-only / absent from Research.
ROOT CAUSE (500): strategy_import.build_draft assumed field types — a numeric `name`, non-list `timeframes`, or non-dict `declarative` raised AttributeError/TypeError, which the global exception handler masked as a generic 500. The /library/import/direct endpoint didn't guard build_draft.
FIXES (backend, shared web+mobile):
- Hardened build_draft: coerce name->str, timeframes->list, conversion/declarative->dict; validate_declarative() coerces non-dict input to {}.
- Guarded POST /library/import/direct: invalid JSON -> 422 with guidance; build failure -> 422 (never a raw 500); insert failure -> clear 500 message.
- NEW power-user path: a JSON already in Ananta declarative rule format ({indicators:{id:{fn,..}}, entry:[{lhs,op,rhs}], exit:[...], params:{}}) is lifted into a compilable declarative block, human-readable entry/exit bullets are synthesized (so review-validation passes), and indicators are normalized to the display list. Result: the strategy becomes declarable=True -> approvable -> appears in Research registry -> deployable to PAPER/LIVE + Lab-runnable, with NO AI.
- Free-form JSON (no engine rules) still saves but as declarable=False (view-only blueprint), now clearly messaged in web + mobile: "Save without AI keeps a view-only blueprint unless your JSON is in Ananta rule format; use Analyze with AI to auto-convert."
VERIFIED (preview, e2e curl): free-form JSON -> HTTP 200 blueprint (was 500); declarative JSON -> declarable=True, approve OK, in registry=True, deploy=PAPER. pytest tests/test_import_direct_robustness.py 2/2 (8 bad-type variants never crash + declarative compiles).
NOTE: user is on PRODUCTION (livetrading247.com / spot-trading-lab.emergent.host) — must REDEPLOY to pick up these preview fixes.
